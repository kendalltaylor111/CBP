...
def validate_high_fidelity_ratios(
    ...
    Requires N=2^20.  Called only from --full.
    """
    results = []
    ...
    for alpha, threshold in thresholds.items():
        ...
        if math.isfinite(Pd) and math.isfinite(Pc) and Pc > 0:
            ...
            results.append(_result(
                False, f"alpha={alpha}: ratio > {threshold}x at eps=1e-3", "Section 11",
                "NaN (eps_target outside data range)", f"> {threshold}x",
                "Check signal length / achievable RMSE range",
            ))
    return results
